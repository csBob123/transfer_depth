...
#             h = output_images[0].shape[0]
#             output_intrinsics[1, 2] = h - output_intrinsics[1, 2]
#         else:
#             output_images = images
#             output_intrinsics = intrinsics
#         return output_images, output_intrinsics
...
#         # scaled_images = [np.array(Image.fromarray(im.astype(np.uint8)).resize((scaled_w, scaled_h), Image.BILINEAR)).astype(np.float32) for im in images]
        
#         scaled_images = []
#         for im in images:
#             if im.ndim < 3: # depth
#                 scaled_images.append(cv2.resize(im, dsize=(scaled_w, scaled_h), interpolation=cv2.INTER_NEAREST))
#             else:
#                 scaled_images.append(cv2.resize(im, dsize=(scaled_w, scaled_h), interpolation=cv2.INTER_CUBIC))
#                 # scaled_images.append(np.array(Image.fromarray(im.astype(np.uint8)).resize((scaled_w, scaled_h), Image.BILINEAR)).astype(np.float32))
...
class RescaleTo(object):
    ...
    def __init__(self, output_size = [192,256]):
    ...
    def __call__(self, images, intrinsics):
        ...
        if in_h != out_h or in_w != out_w:
            ...
            for im in images:
                if im.ndim < 3: # depth
                    scaled_images.append(cv2.resize(im, dsize=(out_w, out_h), interpolation=cv2.INTER_NEAREST_EXACT))
                else:
                    scaled_images.append(cv2.resize(im, dsize=(out_w, out_h), interpolation=cv2.INTER_LINEAR_EXACT))
        else:
