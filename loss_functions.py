...
from inverse_warp import inverse_warp2, pose_vec2mat
...
def compute_pairwise_loss(tgt_img, ref_img, tgt_depth, ref_depth, pose, intrinsic, with_ssim, with_mask, with_auto_mask, padding_mode):
    ...
    geometry_consistency_loss = mean_on_mask(diff_depth, valid_mask*(weight_mask>0.9).float())
...
def compute_pose_loss(poses, poses_gt):
    loss = 0
    pair = 0
    for p, p_gt in zip(poses, poses_gt):
        p_mat = pose_vec2mat(p)
        loss+=(p_mat - p_gt).abs().mean()
        pair +=1
    loss /= pair
    return loss
...
def compute_errors(gt, pred, dataset):
    ...
    for current_gt, current_pred in zip(gt, pred):
        ...
        valid_pred = valid_pred.clamp(0, max_depth)
    ...
    return [metric.item() / batch_size for metric in [abs_diff, abs_rel, sq_rel, a1, a2, a3, scale]]
