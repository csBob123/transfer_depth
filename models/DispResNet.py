...
class DepthDecoder(nn.Module):
    def __init__(self, num_ch_enc, scales=range(4), num_output_channels=1, use_skips=True):
        ...
        self.alpha = 1 / train_min_depth - 1 / train_max_depth
        self.beta = 1 / train_max_depth
    ...
    def forward(self, input_features):
        ...
        for i in range(4, -1, -1):
            ...
            if i in self.scales:
                # disp = self.alpha * self.sigmoid(self.convs[("dispconv", i)](x)) + self.beta

                z = self.convs[("dispconv", i)](x).clamp(-4, 4)
                depth = torch.exp(z).clamp(0.25, 20)
                disp = 1.0 / depth
